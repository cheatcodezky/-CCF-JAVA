...
public class SMS4 {
	...
	//private static byte[] LSbox={(byte) 0xc0,0x50,0x60,(byte) 0xb0,(byte) 0x90,0x00,(byte) 0xa0,
	//		(byte) 0xd0,0x30,(byte) 0xe0,(byte) 0xf0,(byte) 0x80,0x40,0x70,0x10,0x20};
	private static byte[] LSbox={ 0x30,(byte) 0x80,(byte) 0xf0, 0x10,(byte) 0xa0,0x60, 0x50,
		(byte) 0xb0,(byte) 0xe0,(byte) 0xd0, 0x40, 0x20,0x70,0x00,(byte) 0x90,(byte) 0xc0};
	
	private static byte[] Sbox={//S��  ��Ϊһ���ֽڱ�ʾ�ķ�Χ��-128-127������16���Ƴ����÷�Χ�Ͳ�����8λ����ʾ�ˣ�0x44��ʾ68�ڱ�ʾ��Χ֮�ڣ���8λ����0xd6��ʾ214���������ǰ���byte��214��һ���ֽ����ʾ��������byte�����޶���Ϊ�ֽ����͡�
	...
	private int[] CK={//�̶�����ȡֵCK(i,j)=(4i+j)*7(mod256)
	...
	private static int Rotl(int x,int y)//ѭ������ |��λ����һ��λ1��Ϊ1
	{
		return x<<y|x>>>(32-y);
	}
	...
	private static int L1(int B)//���Ա任L ѭ������2��10��18��24  ^��λ��򣬲�ͬΪ1
	{
		return B^Rotl(B,2)^Rotl(B,10)^Rotl(B,18)^Rotl(B,24);
	//	return B^(B<<2|B>>>30)^(B<<10|B>>>22)^(B<<18|B>>>14)^(B<<24|B>>>8);
	}
	
	private int L2(int B)//���Ա任L'��ѭ������13��23
	{
	...
	{//�ӽ����㷨
		...
		{//���������ĵ�һ������128λ��Ϊ4������ԣ����4����ת����һ����4���ֽڣ���ת����16���ֽ�
	...
	{
		byte round_counter=1;
	    int mid;
		byte[] round_key=new byte[10];
		byte[] temp=new byte[3];
		//1
		round_key[9] = (byte) ((Key[6]&0xff) << 5 | (Key[7]&0xff) >> 3);
		round_key[8] = (byte) ((Key[5]&0xff) << 5 | (Key[6]&0xff) >> 3);
		round_key[7] = (byte) ((Key[4]&0xff) << 5 | (Key[5]&0xff) >> 3);
		round_key[6] = (byte) ((Key[3]&0xff) << 5 | (Key[4]&0xff) >> 3);
		round_key[5] = (byte) ((Key[2]&0xff) << 5 | (Key[3]&0xff) >> 3);
		round_key[4] = (byte) ((Key[1]&0xff) << 5 | (Key[2]&0xff) >> 3);
		round_key[3] = (byte) ((Key[0]&0xff) << 5 | (Key[1]&0xff) >> 3);
		round_key[2] = (byte) ((Key[9]&0xff) << 5 | (Key[0]&0xff) >> 3);
		round_key[1] = (byte) ((Key[8]&0xff) << 5 | (Key[9]&0xff) >> 3);
		round_key[0] = (byte) ((Key[7]&0xff) << 5 | (Key[8]&0xff) >> 3);
	    //2
		//round_key[0] = (byte) ((round_key[0] & 0x0f) | (LSbox[round_key[0]>>4]<<4));
		round_key[0] = (byte) ((round_key[0] & 0x0f) | ((LSbox[round_key[0]&0xff>>4]&0xff)<<4));
		round_key[0] = (byte) ((round_key[0] & 0xf0) | LSbox[round_key[0]&0x0f]);
		
		//3
		round_key[7] ^= round_counter >> 1;
		round_key[8] ^= round_counter << 7;
		
		rk[0]=(round_key[0]&0xff<<24)|(round_key[1]&0xff<<16)|(round_key[2]&0xff<<8)|(round_key[3]&0xff);
		
		for(round_counter=2;round_counter<=ROUND;round_counter++)
		{
			temp[2] = round_key[9];
			temp[1] = round_key[8];
			temp[0] = round_key[7];

			round_key[9] = (byte) ((round_key[6]&0xff) << 5 | (round_key[7] &0xff)>> 3);
			round_key[8] = (byte) ((round_key[5]&0xff) << 5 | (round_key[6]&0xff) >> 3);
			round_key[7] = (byte) ((round_key[4]&0xff) << 5 | (round_key[5]&0xff) >> 3);
			round_key[6] = (byte) ((round_key[3]&0xff) << 5 | (round_key[4]&0xff) >> 3);
			round_key[5] = (byte) ((round_key[2]&0xff) << 5 | (round_key[3]&0xff) >> 3);
			round_key[4] = (byte) ((round_key[1]&0xff) << 5 | (round_key[2]&0xff) >> 3);
			round_key[3] = (byte) ((round_key[0]&0xff) << 5 | (round_key[1]&0xff) >> 3);
			round_key[2] = (byte) ((temp[2]&0xff) << 5 | (round_key[0]&0xff) >> 3);
			round_key[1] = (byte) ((temp[1]&0xff) << 5 | (temp[2]&0xff) >> 3);
			round_key[0] = (byte) ((temp[0]&0xff) << 5 | (temp[1]&0xff) >> 3);
			
			

			//round_key[0] = (byte) ((round_key[0] & 0x0f) | LSbox[(round_key[0]&0xff) >> 4]);
			round_key[0] = (byte) ((round_key[0] & 0x0f) | ((LSbox[round_key[0]&0xff>>4]&0xff)<<4));
			round_key[0] = (byte) ((round_key[0] & 0xf0) | LSbox[round_key[0]&0x0f]);
			...
			rk[round_counter-1]=(round_key[0]&0xff<<24)|(round_key[1]&0xff<<16)|(round_key[2]&0xff<<8)|(round_key[3]&0xff);
	...
	{//��Կ��չ�㷨
		...
		for(int i=0;i<4;i++)
		{//��128λ��Կ��Ϊ4��32λ����x[i]
			tmp[0]=Key[0+4*i]&0xFF;//��λ��
			tmp[1]=Key[1+4*i]&0xff;
			tmp[2]=Key[2+4*i]&0xff;
			tmp[3]=Key[3+4*i]&0xff;
			x[i]=tmp[0]<<24|tmp[1]<<16|tmp[2]<<8|tmp[3];//�ĸ��ֽ����һ���֣�ͨ�����Ʋ����ٽ��а�λ����в���
		...
		{//��������Կrk
			...
			mid=ByteSub(mid);           //T�任�������Ƚ���S���û����ٽ������Ա任L
